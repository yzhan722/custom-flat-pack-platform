...
npm test             # 内核 86 个测试 + Web 端 20 个生命周期集成测试
...
| `release` | `buildProductionRelease`：按订单＋版本＋哈希＋序号生成确定性 `releaseKey`，附标签、文件清单、开料单 CSV、操作清单 CSV |
...
| `/design/[orderId]/measure` | FR-02 量尺向导：成品尺寸与可用空间分开、三处宽度、单位换算确认、来源、踢脚线／障碍、内部需求、搬入通道、证据 |
| `/orders/[orderId]` | 订单状态、报价、付款（试点为手工记录、幂等）、生产进度与包件、售后 |
| `/orders/[orderId]/confirm` | FR-07 二维图、可见结构、材料与荷载说明、正式报价、逐项确认与签名；保存快照与哈希 |
| `/orders/[orderId]/assembly` | FR-11 本单装配指南（可打印）、零件与五金袋清单 |
| `/orders/[orderId]/support` | FR-12 按板件／五金袋编号开售后单 |
...
| 报价 | FR-06 服务器重算价格，工程哈希漂移则拒绝；有效期上限 7 天；旧报价自动作废 |
| 收款 | FR-07 收款台账，以银行／网关参考号作幂等键，重复通知只记一次 |
| 放行 | FR-08 门槛逐条列出（确认版本＝报价版本＝批准版本＝当前版本、款项结清、材料与产能确认、标签／包装／教程齐备、无未关闭阻断、确认快照与工程数据一致）；发布包不可覆盖，开料单／操作清单 CSV、标签页、包装单可打印 |
| 车间 | FR-09/10 开始生产、质检（逐板通过／不通过）、包装（称重＋核对）、出库校验（全部板件检验通过、全部包件核对、款项结清、无阻断）、送达、完成、阻断开关、停止发布 |
| 售后 | FR-12 案件状态（已回复／已解决分别记录）、原因分类、责任、成本 |
| 成本 | FR-13 实际成本记录与毛利／CM1／CM2 计算；未记录的类别明确标出，不默认为零 |
| 目录 | 模板、验证包、材料、五金、工厂能力、价目、规则清单（只读） |
...
- **视觉估计不升级**：照片／扫描来源的尺寸永远不会被当作已验证尺寸。

## 测试

- `packages/core/test`：12 个可复算的批准样例（最小／最大组合、1／2／3 模块、门向、开放格、最大跨度、整块／分段顶板、受限空间、自提）；每条阻断规则的触发样例与边界合法样例；确定性与哈希；状态机、支付状态、放行门槛、报价有效性；发布包与 CSV；单位换算；自然语言提议；SHA-256 与 Node 实现比对。
- `apps/web/test`：在内存 PGlite 上跑完整生命周期——门槛拒绝与放行、版本追加、量尺、提交、员工登录、审核、报价、确认快照、幂等付款、放行门槛与发布包、质检与包装阻断、出库、售后、成本贡献；以及变更控制（报价后修改作废报价、`UNSUPPORTED` 不可批准）、访问链接认领、演示数据。
